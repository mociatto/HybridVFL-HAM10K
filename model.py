...
def get_model_variant(train_data, lr=0.001, hyper_gender=0.005, hyper_age=0.001, with_fairness=True):
    ...
    fused = Concatenate()([x, feature_input])  # [image features + tabular features]

    rep_model = Model([image_input, feature_input], fused)
    output_layer = Dense(NUM_CLASSES, activation='softmax')(fused)
